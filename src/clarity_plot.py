...
def detect_inclusions(faceup_path, max_marks=10):
    """Return [(nx, ny, size)] of inclusion blobs, normalized to the outline frame."""
    ...
    ctr = c[:, 0, :].astype(float).mean(0)
    scale = np.abs(c[:, 0, :].astype(float) - ctr).max()
    ...
    out = [((cx - ctr[0]) / scale, -(cy - ctr[1]) / scale, a) for a, (cx, cy) in blobs[:max_marks]]
    return out, (ctr, scale)
...
def build_axes(ax, sid, df=None, model_types=None):
    ...
    for nx, ny, a in blobs:
        ax.plot(nx, ny, marker="o", color="#d11", ms=3 + min(7, a ** 0.5 / 3), alpha=0.85)
