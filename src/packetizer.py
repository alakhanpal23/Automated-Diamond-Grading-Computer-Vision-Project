...
Two-command flow:
    ...
    # 1. Dry run -- write packets to local disk exactly as they'd land in S3, so
    #    you can diff against the platform's reference packet before any AWS.
...
def stone_prefix(parcel_id: str, run_id: str, stone_id: str) -> str:
    """Single source of truth for the platform's S3 key layout (this prefix IS
    part of the verified contract). Every frame key, the metadata key, and the
    idempotency head_object check derive from this so the layout lives in one place."""
    return f"raw/parcels/{parcel_id}/{run_id}/{stone_id}"
...
def build_frame_plan(stone_id: str, frames: list[Path], darkfield_tail: int,
    ...
    """
    n = len(frames)
    # Clamp the tail to [0, n]: a negative flag never darkfields anything, and a
    # tail larger than the frame count just darkfields every frame (rather than
    # producing a negative threshold that silently tags all frames either way).
    tail = max(0, min(darkfield_tail, n))
    df_start = n - tail
    prefix = stone_prefix(parcel_id, run_id, stone_id)
    plan = []
    for idx, _path in enumerate(frames):
        ...
        s3_key = f"{prefix}/{lighting}_{idx}.jpg"
        plan.append({
            "local_path": _path,
            "s3_key": s3_key,
            "media": {
...
def dest_metadata_path_local(out_dir: Path, parcel_id: str, run_id: str,
                             stone_id: str) -> Path:
    return out_dir / stone_prefix(parcel_id, run_id, stone_id) / "metadata.json"
...
def _put_frame(s3, bucket: str, item: dict) -> int:
    ...
    for attempt in range(1, UPLOAD_RETRIES + 1):
        try:
            ...
            # Accept any 2xx as a confirmed write -- S3 returns 200 for PutObject
            # but S3-compatible endpoints (MinIO, etc.) may answer 204.
...
def upload_packet_s3(s3, bucket: str, stone_id: str, row, plan: list[dict],
    ...
    total = 0
    # Frames first, in parallel, each confirmed by a 2xx put response.
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(_put_frame, s3, bucket, item) for item in plan]
    ...
    meta_key = f"{stone_prefix(parcel_id, run_id, stone_id)}/metadata.json"
    meta_bytes = json.dumps(meta, indent=2).encode("utf-8")
    resp = s3.put_object(Bucket=bucket, Key=meta_key, Body=meta_bytes,
                         ContentType="application/json")
    status = resp.get("ResponseMetadata", {}).get("HTTPStatusCode")
    if status is None or not (200 <= status < 300):
        raise RuntimeError(f"metadata.json upload failed for {stone_id} (HTTP {status})")
    total += len(meta_bytes)
    return total
...
def metadata_exists_s3(s3, bucket: str, parcel_id: str, run_id: str,
    ...
    key = f"{stone_prefix(parcel_id, run_id, stone_id)}/metadata.json"
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--log-csv", type=Path, default=LOG_CSV,
                   help="Where to append the per-stone run log.")
    return p.parse_args()
...
def main() -> None:
    ...
    # One row per stone is a hard assumption downstream: we index by stone_id and
    # df.loc[sid] must return a Series, not a DataFrame. A duplicate id would make
    # row.get(col) return a Series, silently emptying measurements/business on an
    # otherwise-"ok" packet. Drop dupes loudly rather than commit bad data.
    n_dups = int(df["stone_id"].duplicated().sum())
    if n_dups:
        print(f"WARN: {n_dups} duplicate stone_id row(s) in CSV; keeping first of each.")
        df = df.drop_duplicates(subset="stone_id", keep="first")

    stones = select_stones(args, df, args.frames_dir)
    if not stones:
        sys.exit("ERROR: no eligible stones selected.")
    # Single under-supply warning covering both the plain and stratified --sample
    # paths (stratified used to top up silently when N exceeded the eligible pool).
    if args.sample is not None and len(stones) < args.sample:
        print(f"WARN: requested --sample {args.sample} but only {len(stones)} "
              f"eligible stones available; using all of them.")
    ...
    for sid in stones:
        ...
        try:
            if args.out_dir is not None:
                ...
                dest = f"s3://{args.bucket}/{stone_prefix(args.parcel_id, args.run_id, sid)}"
                if (not args.force
    ...
    append_log(log_rows, args.log_csv)
    ...
    print("=" * 52)
    print(f"log appended -> {args.log_csv}")
    if n_failed:
        sys.exit(1)
